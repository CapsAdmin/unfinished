...
-- write
local buf = bitbuffer.Writer()
	buf:WriteFloat(123.124124)
	buf:WriteString("himan")
	buf:WriteLong(129381928391823)

	print(buf:ToString())

-- read
local buf = bitbuffer.Reader(buf:ToString())
	local flt = buf:ReadFloat()
	local str = buf:ReadString()
	local lng = buf:ReadLong()
